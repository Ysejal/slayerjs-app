...
const trialOptionMade = (choice, dispatch, player) => {
  ...
  if (result > choice.odds) {
    if (choice.reward === "maxHealth") {
      ...
      text.innerHTML =
        "Success, max health of party increased. ❤️" +
        prev +
        " > >  ❤️" +
        newMax;
    }
    ...
    if (choice.reward === "wealth") {
      ...
      text.innerHTML =
        "Success, the party aquired great fortune. G" +
        g +
        " + " +
        choice.reward_value;
    }
    ...
    if (choice.lose === "curse") {
      for (let i = 0; i < choice.lose_value; i++) {
        dispatch(
          addCard(
            afflictionCards.find((affliction) => affliction.name === "Curse")
          )
        );
      }
      text.innerHTML =
        "The party accidently grabbed a cursed relic." +
        choice.lose_value +
        " curses have been added to the parties deck.";
    }
  }
};
    ...
    <div className={styles.screenContainer}>
      <div id="resultWindow" className={styles.resultWindow}>
        <div className={styles.reultText}>
          ...
          <button
            className={styles.trialOptions}
            onClick={() => continueButton(dispatch)}
          >
            continue
          </button>
        </div>
          ...
          <div className={styles.trialInfoContainer}>
            ...
            <div className={styles.trialOptionsContainer}>
              {trial.options.map((option) => (
                <button
                  className={styles.trialOptions}
                  onClick={() => trialOptionMade(option, dispatch, player)}
                >
                  {option.description}
                </button>
              ))}
